...
export const ParticipantRenderer = ({
    participant,
    participants,
}: {
    participant: any;
    ...
    const { isSpeaking, subscribedTracks, isLocal } =
        ...
        <Grid item xs={12} md={participants.length > 1 ? 6 : 12}>
            <Box
                ...
                sx={{
                    ...
                    alignItems: 'center',
                    border: isSpeaking ? '1px #f00 solid' : null,
                }}
            >
                {subscribedTracks[1]?.track ? (
                    <VideoRenderer
                        track={subscribedTracks[1].track}
                        isLocal={true}
                        width='100%'
                        height='100%'
                        objectFit='cover'
                ...
                {subscribedTracks[0]?.track ? (
                    <AudioRenderer
                        track={subscribedTracks[0].track}
                        isLocal={isLocal}
                    />
                ) : null}
            </Box>
...
const Room = () => {
    ...
    const { url, userId, roomName, livekiturl } = router.query;
    ...
    }, [connect, livekiturl, room?.localParticipant, token]);
